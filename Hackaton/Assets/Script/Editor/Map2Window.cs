...
public class Map2Window : EditorWindow
{
    ...
    private void BuildCache()
    {
        ...
        List<CachedNode> nodes = allTypes
            .Select(t => new CachedNode { Type = t, Attr = t.GetCustomAttribute<MapNodeAttribute>() })
            .Where(x => x.Attr != null)
            .ToList();
    ...
    private bool DrawFoldout(CachedNode node, string path)
    {
        ...
        bool newExpanded = EditorGUILayout.Foldout(
            isExpanded,
            $"{node.Attr.DisplayName}  [{node.Attr.Tags}]",
            true,
            foldoutStyle
        );
    ...
    void DrawNode(CachedNode node, int indent, HashSet<CachedNode> visited, string path)
    {
        ...
        List<NodeLink> links = node.Outgoing.OrderBy(l => l.Target.Attr.DisplayName).ToList();
        ...
        foreach (NodeLink link in links)
        {
            ...
            EditorGUILayout.LabelField($"Uses: {link.Target.Attr.DisplayName}{usesTxt}");
        }
